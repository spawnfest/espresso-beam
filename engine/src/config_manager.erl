%%%-------------------------------------------------------------------
%%% File    : config_manager.erl
%%% Author  : Paolo D'Incau <paolo.dincau@gmail.com>
%%% Description : 
%%%
%%% Created : 7-5-2012 by Paolo  <paolo.dincau@gmail.com>
%%%-------------------------------------------------------------------
...
%%--------------------------------------------------------------------
init([]) ->
    {ok, Conf} = file:consult("priv/espresso_beam.cfg"),
    {ok, #state{conf=Conf}}.
...
handle_call(revert, _From, State) ->
    {ok, NewConf} = file:consult("priv/espresso_beam.cfg"),
    Reply = ok,
    {reply, Reply, State#state{conf=NewConf}};
